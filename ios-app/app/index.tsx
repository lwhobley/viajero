...
function VoicePractice({ targetPhrase, onResult }: { targetPhrase?: string; onResult: (transcript: string, comparison?: PhraseComparison) => void }) {
  ...
  useEffect(() => {
    if (status === 'listening') { wasListening.current = true; setResult(null); }
...
export default function HomeScreen() {
  ...
  const [messages, setMessages] = useState<{ text: string; from: 'server' | 'you' }[]>([{ text: 'Buenas tardes. ¿Qué le gustaría?', from: 'server' }]);
  ...
  const sendReply = (spokenText?: string) => {
    const text = (spokenText ?? reply).trim(); if (!text) return;
    setMessages((items) => [...items, { text, from: 'you' }]); setReply(''); completeTask();
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" style={styles.page} contentContainerStyle={styles.content}>
    ...
    {tab === 'conversation' && <View style={styles.section}><View style={styles.sectionHeader}><View><Text style={styles.eyebrow}>LIVE PRACTICE</Text><Text style={styles.sectionTitle}>Talk to the server</Text></View><Text style={styles.ready}>● Voice ready</Text></View><View style={styles.conversation}>{messages.map((message, index) => <View key={`${message.text}-${index}`} style={[styles.bubble, message.from === 'you' ? styles.youBubble : styles.serverBubble]}><Text style={styles.bubbleText}>{message.text}</Text><Text style={styles.bubbleLabel}>{message.from === 'you' ? 'TÚ' : 'SERVIDOR'}</Text></View>)}</View><Text style={styles.helper}>Type an answer or record yourself. The server still speaks back in Spanish.</Text><View style={styles.inputRow}><TextInput value={reply} onChangeText={setReply} onSubmitEditing={() => sendReply()} placeholder="Escribe tu respuesta..." placeholderTextColor="#9a9d94" style={styles.input} /><Pressable onPress={() => sendReply()} style={styles.send}><Text style={styles.sendText}>Send</Text></Pressable></View><VoicePractice onResult={(transcript) => sendReply(transcript)} /></View>}
